...
// drawNinePatches stretches the ui source image onto the dest image. it splits the source image into a grid.
func drawNinePatches(dst *ebiten.Image, dstRect image.Rectangle, srcRect image.Rectangle) {
	op := &ebiten.DrawImageOptions{}

	// get source top/left and width/height
	srcX := srcRect.Min.X
	srcY := srcRect.Min.Y
	srcW := srcRect.Dx()
	srcH := srcRect.Dy()

	// get destination top/left and width/height
	dstX := dstRect.Min.X
	dstY := dstRect.Min.Y
	dstW := dstRect.Dx()
	dstH := dstRect.Dy()
	...
	for j := 0; j < 3; j++ {
		...
		for i := 0; i < 3; i++ {
			...
			// get source sub-image
			sx := srcX
			sy := srcY
			sw := srcW / 4
			sh := srcH / 4

			// set destination sub-image
			dx := 0
			dy := 0
			dw := sw
			dh := sh
